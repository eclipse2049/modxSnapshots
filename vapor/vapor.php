...
try {
    ...
    if (!defined('PKG_VERSION')) define('PKG_VERSION', strftime("%y%m%d.%H%M.%S", $startTime));
    if (!defined('PKG_RELEASE')) define('PKG_RELEASE', $modxVersion);
    ...
    $sources = array(
        'core_path'  =>  $shopPath = MODX_CORE_PATH .'components/shop/'
    );
    $sources['docs'] = $sources['core_path']. 'docs/';
    ...
    /**
     * Добавляем core/helpers/, core/elements/, core/import/
     */
    $modx->log(modX::LOG_LEVEL_INFO, "Packaging " . MODX_CORE_PATH . 'helpers');
    $package->put(
        array(
            'source' => MODX_CORE_PATH . 'helpers',
            'target' => 'return MODX_CORE_PATH;'
        ),
        array(
            'vehicle_class' => 'xPDOFileVehicle'
    ...
    $modx->log(modX::LOG_LEVEL_INFO, "Packaging " . MODX_CORE_PATH . 'elements');
    $package->put(
        array(
            'source' => MODX_CORE_PATH . 'elements',
            'target' => 'return MODX_CORE_PATH;'
        ),
        array(
            'vehicle_class' => 'xPDOFileVehicle'
    ...
    $modx->log(modX::LOG_LEVEL_INFO, "Packaging " . MODX_CORE_PATH . 'import');
    $package->put(
        array(
            'source' => MODX_CORE_PATH . 'import',
            'target' => 'return MODX_CORE_PATH;'
        ),
        array(
            'vehicle_class' => 'xPDOFileVehicle'
    ...
    if (isset($vaporOptions['excludeFiles']) && is_array($vaporOptions['excludeFiles'])) {
        foreach($vaporOptions['excludeFiles'] as $excludeFile){
            if(!in_array($excludeFile, $excludes)){
        ...
        array(
            ...
            'validate' => array(
                array(
                    ...
                    'classes' => array_diff($classes, $excludeClasses), // $classes
                ),
            ),
            'resolve' => array(
                array(
    ...
    foreach (array_diff($classes, $excludeClasses) as $class) {
        if (!XPDO_CLI_MODE && !ini_get('safe_mode')) {
            set_time_limit(0);
        }
